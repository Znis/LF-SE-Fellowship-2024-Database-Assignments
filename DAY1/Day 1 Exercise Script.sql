...
-- Read products that are in Electronics category from Products table
select customer_name from Orders where order_date between '2024-06-01' and '2024-06-03';
...
update Orders set quantity = 4 where order_id = 1;


-- Delete the Appliances category from Products table
select * from Products where category = 'Appliances'; -- verify the records to be deleted
delete from Products where category = 'Appliances';

-- Delete the order_id 4 from Orders table
select * from Orders where order_id = 4; -- verify the records to be deleted
delete from Orders where order_id = 4;

-- Qno.2 Calculate the total quantity ordered for each product category in the Orders table
select p.category, sum(o.quantity) as totalQuantity from products p
join orders o on p.product_id = o.product_id 
group by p.category
order by sum(o.quantity) desc;

-- Qno.3 Find categories where the total number of products ordered is greater than 5
select p.category, count(p.category) as totalProductsOrdered from products p
join orders o on p.product_id = o.product_id 
group by p.category
